...
<?php $id='sem010'; $nav=1; $nav_items=array('ex','reqs','faq'); $category='semantics'; $page_name='Logical Reading Order'; ?>
<!DOCTYPE html>
	...
	<body>
		...
		<section id="sem010" class="section">
			...
			<section id="sem010-desc" class="usage">
				<p>Publications typically have a primary narrative that readers are expected to follow from beginning 
					to end, and being able to navigate this dialogue uninterrupted is a key factor in 
					making publications accessible.</p>
				
				<p>There is sometimes no distinction between the narrative and its representation in the markup. A novel, 
					for example, might consist only of sections, headings and paragraphs which are all marked up in 
					order and can be read in order. These cases tend not to be the norm, however, as most publications 
					will have some degree of complexity to their layout.</p>
				
				<p>More often, publications will include at least some supplementary information, such as footnotes, 
					endnotes, references, sidebars, tables, figures, charts, code samples, warnings, alerts and similar.
					As this information is interspersed within the content of the body, its markup must inevitably also
					be interspersed within a content document (e.g., a sidebar may be placed between two paragraphs within 
					a section and the text visually floated around it). If this content is not clearly marked and identifiable
					as secondary information, readers navigating at the markup level will have the primary narrative interrupted
					as the reading system attempts to play back the auxiliary content.</p>
				
				<p>This problem was particularly acute prior to HTML5, as generic <code>div</code> tags we used to group secondary material, 
					which left no way to reliably distinguish its purpose. The result was that readers manually had to try
					and determine where the content ended in order to continue reading.</p>
				...
				<ul>
					<li>that all complex structures like lists, tables, figures, code samples and the like been
						properly tagged using the appropriate structural elements;</li>
					<li>that the <code>section</code> and <code>aside</code> elements have been appropriately applied 
						to groupings of primary and secondary content, respectively;</li>
					<li>that the appropriate semantics have been applied to all elements to disambiguate their use;</li>
					<li>that the order of content within the markup accurately reflects the natural reading order (i.e.,
						CSS is not being used to reposition elements in a different sequence for visual readers 
						than can be found in the markup);</li>
					<li>that JavaScript is not required to be enabled in order to read a document from beginning to end
						(e.g., scripted links, if present, only enhance standard links, interactive puzzles and games
						are not required to be solved to move on, etc.).</li>
				</ul>
				...
				<p>Although creating the logical reading order is not a complicated requirement, it typically requires manual 
					...
					secondary can only be reliably determined by a human, so a successful validation report from a tool like 
					<a href="http://code.google.com/p/epubcheck/">epubcheck</a>, while helpful, is only a good start.</p>
			...
			<section id="sem010-ex" class="example">
				...
				<figure id="sem010-ex01">
					<figcaption>Example 1 &#8212; Semantic markup to enable the discovery of the logical reading order</figcaption>
					...
					<img src="../../graphics/order.jpg" alt="Sample textbook page showing figures and asides surrounding the narrative"/>
					<p>The logical reading order would be established using <code>figure</code> tags to offset the 
						images and an <code>aside</code> for the sidebar, as in the following markup:</p>
...
&lt;/aside>
			...
			<section id="sem010-faq" class="faq">
				...
				<dl class="faq">
					...
					<dd><p>Consider how someone reading using a text-to-speech engine interacts with your
							publication. They are navigating at the markup level, moving from structure to structure and hearing
							the prose rendered at each step. That's fine, but now picture having no information about the structure
							you've just reached. Are you going to the next paragraph, or is a footnote suddenly going to be read
							back? Did the last section end or did you just jump into a sidebar?</p>
						
						<p>Without structure and semantics, there's no way to easily move from one structure to the next. 
							Where does the sidebar marked as a <code>div</code> end and the next paragraph continue? 
							These questions left to the reader to puzzle out, leaving them manually navigating the 
							publication to fit together the elusive reading order from the generic markup. It's also 
							why semantically meaningless markup ruins the reading experience, if not makes it downright 
							impossible.</p>
					</dd>
					...
					<dd><p>Accessible reading systems typically provide the option for readers to select elements to ignore.
							Semantic markup eases this process of identifying and skipping non-essential content. A
							reader may still choose to render secondary content as it occurs, but it becomes their choice
							again, and no longer a burden to navigate through or around as desired.</p></dd>
				</dl>
			</section>
		</section>
